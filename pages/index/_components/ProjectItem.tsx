...
export default (props: Props) => {
  ...
  return (
    <a
      class="relative rounded-2xl bg-surface/60 backdrop-blur-sm border border-stroke transition-all decoration-none hover:bg-stroke/40 hover:-translate-y-0.5 group flex flex-col h-full overflow-hidden"
      href={data.link}
      target="_blank"
    >
      {data.showcase && (
        <div class="w-full aspect-[1200/630] bg-stroke/30 overflow-hidden border-b border-stroke">
          <img
            src={data.showcase}
            alt={data.name}
